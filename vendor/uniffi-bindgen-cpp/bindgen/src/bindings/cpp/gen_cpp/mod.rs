...
impl<'a> CppWrapperHeader<'a> {
    ...
    pub(crate) fn sorted_types(
        ...
        let mut definition_topology = self
            ...
            // Self-references (e.g. `[Enum] LoroValue { List(sequence<LoroValue>); ... }`)
            // surface as `prec == succ`, which the underlying TopologicalSort
            // treats as a 1-node cycle and panics on. Recursive types are
            // handled in C++ via `std::shared_ptr` indirection inside the
            // generated variant; the self-edge is not a real ordering
            // constraint.
            .filter(|d| d.prec != d.succ)
...
fn type_name(ty: &Type) -> Option<&str> {
    match ty {
        ...
        // Objects only need forward declarations in C++ (they always live
        // behind `std::shared_ptr`), so omit them from the topological
        // sort. If we let an Object participate as a leaf node, it would
        // be popped *before* the Records / Enums it transitively depends
        // on, producing class definitions whose method return types
        // (e.g. `std::optional<Record>`) reference incomplete classes.
        Type::Object { .. } => None,
        _ => None,
    }
}
