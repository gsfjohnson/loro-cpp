...
impl CppCodeOracle {
    ...
    pub(crate) fn fn_name(&self, nm: &str) -> String {
        // Methods can collide with C++ keywords (Loro's UDL has
        // `void delete(...)` on LoroText, for example). Treat reserved
        // words the same way as variables: append a trailing underscore.
