...
function(_generator_add_package_targets)
    ...
    # loro-cpp patch: two issues with cargo-metadata-supplied paths under
    # MSYS2 cmake:
    #   (a) `file(TO_CMAKE_PATH)` is documented for path *lists* — `:` is its
    #       list separator on Unix-flavored cmake (which MSYS2 cmake is, even
    #       though it ships as a Windows binary). Feeding it a single Windows
    #       path like `C:\Users\…` mangles `C:` to `C;`, which then breaks
    #       `cmake_parse_arguments` downstream.
    #   (b) MSYS2 cmake's `IS_ABSOLUTE` only recognizes Unix-style absolute
    #       paths (leading `/`). It treats `C:/Users/…` as relative and the
    #       `_add_cargo_build` IS_ABSOLUTE check then prepends CMAKE_SOURCE_DIR,
    #       producing a doubled-up path that breaks both `cd` and
    #       `--manifest-path` in the generated cargo build script.
    # Fix: backslash→slash, plus convert `<drive>:/…` to MSYS form `/<drive>/…`.
    # The MSYS form is recognized as absolute by every cmake, and MSYS2's
    # automatic path-translation rewrites it back to Windows form when cargo
    # (a Windows binary) is invoked from sh.
    string(REPLACE "\\" "/" manifest_path "${package_manifest_path}")
    if (manifest_path MATCHES "^([A-Za-z]):/(.*)$")
        string(TOLOWER "${CMAKE_MATCH_1}" _drive)
        set(manifest_path "/${_drive}/${CMAKE_MATCH_2}")
        unset(_drive)
    endif()
